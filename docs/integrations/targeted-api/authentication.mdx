...
Before you can authenticate, you need to obtain your `clientId` and `clientSecret` from the AdGem dashboard. These credentials are unique to your application.
...
curl -X POST https://targeted-api.adgem.com/v1/apps/tokens \
  ...
  -d '{
    "adgem_pub_id": <your-publisher-id>,
...
See the [Tokens endpoint documentation](/targeted-api/tokens) for more details.
