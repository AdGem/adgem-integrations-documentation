...
3. **Verify the `Signature` header** instead of the `verifier` query parameter: compute `HMAC-SHA256(raw_request_body, your_secret_key)` and compare. (v2 hashes the URL; v3 hashes the JSON body.) See [Postbacks (v3)](/docs/integrate/reward-mechanism/postbacks-v3).
