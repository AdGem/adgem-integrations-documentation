...
2.  **New Offer Insertion:** Offers marked as "New" (which typically have an EPC of $0.00$) are removed from the bottom of the list and **re-inserted starting at Position 5**.
...
Below is an example illustrating how a set of offers transitions from the initial state to the final, displayed order.
...
Offers are sorted strictly by `network_epc` (Descending). New offers (EPC $0.00$) fall to the bottom.
...
## How does the translation fallback works
