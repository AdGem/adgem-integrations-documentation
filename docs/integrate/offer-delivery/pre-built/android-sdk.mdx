...
description: Integrate the AdGem Offerwall into your Android application using the native Android SDK.
---
...
<BeforeYouBegin platform="android" />

:::tip Unity Games
Is your Android game built in Unity? We strongly recommend integrating the [AdGem Unity SDK](/docs/integrate/offer-delivery/pre-built/unity-sdk) for both Android and iOS games built with Unity.
:::
...
```java
adGem.showOfferwall(this); // Show Offerwall (pass your Activity as the Context)
```

### Step 4. Register the AdGem Offerwall Callback

The AdGem SDK provides callbacks that notify when Offerwall internal state changes.
...
Offerwall callback may be registered through the instance of `AdGem`:
...
Offerwall can be displayed by calling `adGem.showOfferwall(activity)`.
...
The AdGem Android SDK allows for several optional parameter values to be stored such as age, gender, etc. These values can then be retrieved again on each conversion postback and used to segment your audiences and optimize your mobile ad revenue earnings.
