...
The reporting data source refreshes on a **30-minute interval**, so frequent polling returns no new data. Keep requests at or below **10 per minute** — query on a schedule (for example, hourly) and cache the results.
