...
**Open questions / risks:**
- **DEFERRED to WS2 start (2026-06-09):** the remap table surfaced 6 IA gaps with no clean home — resolve these *before* bulk authoring. See `docs/plans/2026-06-09-ws2-remap-table.md` §Gaps. Summary: (1) **payments/monetization has no surface** — likely needs a Gus conversation; (2) per-SDK subpages (changelog/example-app/optional-parameters) vs single SDK pages; (3) offer-concept pages → Get Started core-concepts (+ difficulty-scale placement); (4) Prism FAQ has no slot; (5) Resources extras (developer-support, product-roadmap, support-urls) keep/externalize/drop; (6) webhook-events Configure-vs-Reference split. Also: Web Offerwall customization duplicates the Configure customization page (dedupe), and `legal/sdk-license-agreement` has no external URL (Iliana).
